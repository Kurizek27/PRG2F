...
//        }
        ...
        for (int i = 0; i < newRatings.length; i++) {
            if (newRatings[i] > newRatings[highestRating]) {
             highestRating = i;
            }
        }
        System.out.println("Nejlepší hodnocení:");
        System.out.println(newSongs[highestRating] + " " + newRatings[highestRating]);
        int lowestRating = 0;
        for (int i = 0; i < newRatings.length; i++) {
            if (newRatings[i] < newRatings[lowestRating]) {
                lowestRating = i;
            }
        }
